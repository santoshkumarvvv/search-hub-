...
import { getHindiDubbed, searchAnime } from '@/lib/data';
...
export default function SearchClient() {
  ...
  const [hindiOnly, setHindiOnly] = useState(searchParams.get('hindi') === '1');
  ...
  useEffect(() => {
    ...
    if (hindiOnly) params.set('hindi', '1');
    const qs = params.toString();
    router.replace(qs ? `/search?${qs}` : '/search', { scroll: false });
  }, [debounced, genre, hindiOnly, router]);

  const results = useMemo(
    () => searchAnime(debounced, { genre, hindiOnly }),
    [debounced, genre, hindiOnly],
  );

  const isSearching = debounced.trim().length > 0 || genre !== 'all' || hindiOnly;
  const suggestions = useMemo(() => getHindiDubbed().slice(0, 12), []);
    ...
    <>
      <div className="mt-7">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <button
            ...
            className={cn('chip', hindiOnly && 'chip-active')}
          >
            <Mic size={12} /> सिर्फ़ हिंदी डब
          </button>
        </div>
      ...
      <div className="mt-8">
        ...
        {isSearching && !results.length ? (
          <EmptyState
            title="कुछ नहीं मिला"
            description="कोई और कीवर्ड आज़माएँ या फ़िल्टर हटाकर देखें।"
